...
httpparse = (function () {
	...
	var build_query = function (object) {
		...
		for (var key in object) {
			if (key) {
				var key = escape(key);
				var val = object[key] ? escape(object[key].toString()) : '';
	...
	var polish_url = function (object) {
		...
		if (type.is_number(object.port)) {
			var port  = object.port >= 0 ? object.port : undefined;
		} else
		if (type.is_string(object.port)) {
			try {
	...
	var polish_response = function (object) {
		// TODO
	};
	...
	// POLISH REQUEST
	// polish request object
	var polish_request = function (object) {
		var method = object.method ? object.method.toString().toUpperCase() : 'GET';
		var url = object.url ? object.url.toString() : 'http://localhost';
		var headers = object.query ? object.query.toString() : '';
		var data = object.data ? object.data.toString() : '';
